 ...
 * @author langmo
 */
class StaggeringJobConfigurationAddon implements JobConfigurationAddon
{

	private StaggeringJobConfiguration		job							= null;
	...
	StaggeringJobConfigurationAddon(YouScopeClient client, YouScopeServer server)
	{
		this.client = client;
		this.server = server;
	}
	...
	{
		this.frame = frame;
		frame.setTitle("Staggering Job");
		frame.setResizable(true);
		frame.setClosable(true);
		frame.setMaximizable(false);

		StaggeringJobConfiguration job = getConfigurationData();
		...
		fromCameraButton.addActionListener(new ActionListener()
		{
			...
			{
				YouScopeFrame childFrame = StaggeringJobConfigurationAddon.this.frame.createModalChildFrame();
		...
		addJobButton.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				{
					listener.jobConfigurationFinished(StaggeringJobConfigurationAddon.this.job);
				}
				try
				{
					StaggeringJobConfigurationAddon.this.frame.setVisible(false);
				}
				catch(Exception e1)
				{
					// Should not happen!
		...
		nIterBreakField.setValue(job.getNumIterationsBreak());
		nTilesPerIterField.setValue(job.getNumTilesPerIteration());
	...
	{
		if(!(job instanceof StaggeringJobConfiguration))
			throw new ConfigurationException("Configuration not supported by this addon.");
		this.job = (StaggeringJobConfiguration)job;
	}

	@Override
	public StaggeringJobConfiguration getConfigurationData()
	{
		if(job == null)
		{
			job = new StaggeringJobConfiguration();
		}
		return job;
	}
	...
	private void setConfigDataToUI()
	{
		...
		job.setNumIterationsBreak(nBreak);
		job.setNumTilesPerIteration(nTilesIter);
	...
	{
		return StaggeringJobConfiguration.TYPE_IDENTIFIER;
	}
