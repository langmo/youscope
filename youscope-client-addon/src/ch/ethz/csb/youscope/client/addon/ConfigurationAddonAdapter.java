 ...
 *
 */
public abstract class ConfigurationAddonAdapter<C extends Configuration>  implements ConfigurationAddon<C> 
{
	...
	private ArrayList<ConfigurationAddonListener<? super C>> configurationListeners = new ArrayList<ConfigurationAddonListener<? super C>>();
	...
	private final ConfigurationMetadata<C> metadata;
	 ...
	 */
	public ConfigurationAddonAdapter(final ConfigurationMetadata<C> metadata,  final YouScopeClient client, final YouScopeServer server) throws AddonException 
	{
		this.client = client;
		this.server = server;
	...
	private Component getDescriptionComponent()
	{
		if(description == null)
			return null;
		URL iconURL = ConfigurationAddonAdapter.class.getClassLoader().getResource("icons/question.png");
		JLabel label;
		if (iconURL != null)
		{
	...
	private Component setupPanel() throws AddonException
	{
		...
		class UIPanel extends JPanel
		{
			...
			{
				Dimension preferredSize = ConfigurationAddonAdapter.this.preferredSize;
				boolean resizable = ConfigurationAddonAdapter.this.resizable;
	...
	protected void configurationFinished()
	{
		...
		{
			YouScopeFrame errorFrame = ConfigurationAddonTools.displayConfigurationInvalid(e, getClient());
		...
		synchronized(configurationListeners)
		{
			for(ConfigurationAddonListener<? super C> configurationListener : configurationListeners)
			{
