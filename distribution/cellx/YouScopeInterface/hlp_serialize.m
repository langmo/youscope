...
function m = serialize_object(v)
    contents = cell(1, numel(v));
    for i=1:numel(v)
        try
            % try to use the saveobj method first to get the contents
            contents{i} = saveobj(v(i));
            if isstruct(contents{i}) || iscell(contents{i}) || isnumeric(contents{i}) || ischar(contents{i}) || islogical(contents{i}) || isa(contents{i},'function_handle')
                % contents is something that we can readily serialize
                contents{i} = hlp_serialize(contents{i});
            else
                % contents is still an object: turn into a struct now
                contents{i} = serialize_struct(struct(contents{i}));
            end
        catch
            % saveobj failed for this object: turn into a struct
            contents{i} = serialize_struct(struct(v(i)));
        end
    end
    % Tag, Class name and Contents    
    m = [uint8(134); serialize_string(class(v)); ndims(v); typecast(uint32(size(v)),'uint8').'; vertcat(contents{:})];
end
