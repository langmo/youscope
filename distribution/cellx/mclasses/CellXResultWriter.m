classdef CellXResultWriter < handle
    ...
    methods (Static)
        ...
        function writeTxtSegmentationResults(fSet, seeds, config)
            ...
            jointFluoHeader = [];jointFluoFormat=[];
            for ft=1:nrOfFluoTags
                ...
                if ft>1
                    jointFluoFormat = [jointFluoFormat,'\t',CellXResultWriter.fluoFormat];
                else
                    jointFluoFormat = ['\t',CellXResultWriter.fluoFormat];
                end
            end
            ...
            % write the headers
            for nrh=1:numel(jointHeader)
                if nrh<numel(jointHeader)
                    fprintf(out, '%s\t', jointHeader{nrh});
                else
                    fprintf(out, '%s\n', jointHeader{nrh});
                end
            end
