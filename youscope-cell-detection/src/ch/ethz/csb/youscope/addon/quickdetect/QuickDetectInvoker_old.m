...
if ~isempty(imageEvent)
    ...
    if ~isempty(cellPositions)
    	...
    	cellTableRows = javaArray('ch.ethz.csb.youscope.shared.TableDataRow', size(cellPositionsStr, 1));
    	for i=1:size(cellPositionsStr, 1)
    		cellTableRows(i) = ch.ethz.csb.youscope.shared.TableDataRow(cellPositionsStr(i, :), [], [], java.util.Date());
    	end
    	% Submit to YouScope
