...
import ch.ethz.csb.youscope.shared.microscope.MicroscopeStateListener;
...
public class YouScopeServerImpl implements YouScopeServer
{
	 ...
	 */
	private static final String				POLICY_FILE								= "ch/ethz/csb/youscope/server/YouScope.policy";
	...
	private MicroscopeInternal connectToMicroscope() throws MicroscopeException
	{
		...
		String connectionType = configuration.getProperty(PROPERTY_MICROSCOPE_CONNECTION_TYPE, "CSB::StandAlone14");
		if(configuration.getProperty(PROPERTY_FORCE_CONNECTION_CONFIGURATION, false))
		{
			MicroscopeConnectionTypeChooser connectionTypeChooser = new MicroscopeConnectionTypeChooser(driverFolder, connectionType, null);
		...
		while(true)
		{
			...
			catch(MicroscopeConnectionException e)
			{
				...
				MicroscopeConnectionTypeChooser connectionTypeChooser = new MicroscopeConnectionTypeChooser(driverFolder, connectionType, e);
				connectionTypeChooser.waitForUserInput();
				driverFolder = connectionTypeChooser.getDriverPath();
				connectionType = connectionTypeChooser.getConnectionType();
			}
