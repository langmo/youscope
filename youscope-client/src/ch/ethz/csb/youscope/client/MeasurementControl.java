...
import ch.ethz.csb.youscope.client.uielements.LinkLabel;
...
class MeasurementControl
{
	...
	private void setupUIElements() throws RemoteException
	{
			...
			editMeasurementButton.addActionListener(new ActionListener()
			{
				...
				public void actionPerformed(ActionEvent arg0)
				{
					if(state == MeasurementState.READY || state == MeasurementState.FINISHED || state == MeasurementState.ERROR || state == MeasurementState.UNQUEUED)
					{
						...
						MeasurementConfigurationAddon addon = addonFactory.createMeasurementConfigurationAddon(configuration.getTypeIdentifier(), new YouScopeClientConnectionImpl(), YouScopeClientImpl.getServer());
						...
						{
							ClientSystem.err.println("Error in configuration.", e);
							return;
						}
						YouScopeFrame newFrame = YouScopeFrameImpl.createTopLevelFrame();
						addon.createUI(newFrame);
		...
		Iterator<MeasurementPostProcessorAddonFactory> factories = ClientSystem.getMeasurementPostProcessorAddons().iterator();
		
		for(;factories.hasNext();)
		{
			MeasurementPostProcessorAddonFactory factory = factories.next();
	...
	private class StartProcessorMenuItem extends JMenuItem implements ActionListener
	{
		...
		private final MeasurementPostProcessorAddonFactory addonFactory;
		private final String addonID;
		StartProcessorMenuItem(MeasurementPostProcessorAddonFactory addonFactory, String addonID)
		{
		...
		public void actionPerformed(ActionEvent arg0)
		{
			...
			MeasurementPostProcessorAddon addon = addonFactory.createMeasurementConfigurationAddon(addonID, new YouScopeClientConnectionImpl(), YouScopeClientImpl.getServer(), measurementFolder);
