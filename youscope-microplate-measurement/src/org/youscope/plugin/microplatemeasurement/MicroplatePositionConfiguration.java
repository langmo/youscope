...
public class MicroplatePositionConfiguration extends MicroplateConfiguration
{
	 ...
	 */
	public MicroplatePositionConfiguration(int numWellsX, int numWellsY, double wellWidth, double wellHeight)
	{
	 ...
	 */
	public MicroplatePositionConfiguration(int numPositions)
	{
		super(numPositions);
	}
			...
			for(int wellX = 0; wellX < getNumWellsX(); wellX++)
			{
				...
				for(int posY = 0; posY < getWellNumPositionsY(); posY++)
				{
					for(int posX = 0; posX < getWellNumPositionsX(); posX++)
					{
						...
						setPosition(new XYAndFocusPosition(x, y, zeroPositionFocus), new Well(wellY, wellX), posY, posX);
	 ...
	 */
	public XYAndFocusPosition getPosition(Well well, int posY, int posX)
	{
	...
	{
		MicroplatePositionConfiguration clone = (MicroplatePositionConfiguration)super.clone();
		// Clone hashPositions
		clone.hashPositions = new Hashtable<WellAndTileIdentifier, XYAndFocusPosition>((int)(hashPositions.size()*1.5));
		for(Entry<WellAndTileIdentifier, XYAndFocusPosition> entry : hashPositions.entrySet())
		{
			clone.hashPositions.put((WellAndTileIdentifier)entry.getKey().clone(), (XYAndFocusPosition)entry.getValue().clone());
		}
